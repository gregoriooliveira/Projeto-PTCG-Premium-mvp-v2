...
function sanitizePokemonHintsList(source) {
  ...
  const normalized = normalizePokemonHints(arr);
  if (!Array.isArray(normalized)) return [];
  return normalized
    .map((slug) => (typeof slug === "string" ? slug.trim() : String(slug || "").trim()))
    .filter(Boolean);
}
...
r.post("/events", authMiddleware, async (req, res) => {
  ...
  const doc = {
    eventId, source:"live", createdAt, date,
  ...
  await db.collection("liveEvents").doc(eventId).set(doc, { merge: true });
...
r.get("/events/:id", async (req, res) => {
  ...
  if (ev.rawLogId) {
    try {
      ...
      console.error("[live/events:id] rawLog lookup failed", e);
    }
  }
...
r.patch("/events/:id", authMiddleware, async (req, res) => {
  ...
  await db.collection("liveEvents").doc(id).set(update, { merge: true });
...
r.get("/decks", async (req, res) => {
  ...
  if (deck) {
    ...
    const doc = await db.collection("liveDecksAgg").doc(dk).get();
...
r.get("/opponents-agg", async (req, res) => {
  try {
    ...
    console.error("[GET /live/opponents-agg]", e);
...
r.get("/decks/:deck/logs", async (req, res) => {
  try{
    ...
    const snap = await db.collection("liveEvents")
    ...
    console.error("[GET /live/decks/:deck/logs]", e);
...
r.get("/logs", async (req, res) => {
  try {
    ...
    let ref = db.collection("liveEvents");
