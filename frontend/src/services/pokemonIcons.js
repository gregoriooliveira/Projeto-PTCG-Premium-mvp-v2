...
async function fetchIcon(slug) {
  ...
  const lsKey = LS_PREFIX + slug;
  const cached = typeof localStorage !== "undefined" ? localStorage.getItem(lsKey) : null;
  if (cached) { MEM.set(slug, cached); return cached; }
  try {
    ...
    MEM.set(slug, out);
    if (typeof localStorage !== "undefined") localStorage.setItem(lsKey, out);
    return out;
  } catch {
    MEM.set(slug, FALLBACK);
    if (typeof localStorage !== "undefined") localStorage.setItem(lsKey, FALLBACK);
    return FALLBACK;
  }
}
