...
function isOnlineTournamentLog(r) {
  const flag = r?.isOnlineTournament ?? r?.isTournamentOnline ?? r?.tournamentOnline ?? r?.isTournament ?? r?.onlineTournament ?? r?.tourneyOnline;
  if (flag != null) return truthy(flag);
  const t = r.tournament || r.tourney || r.event || {};
  return Boolean(
    r.tournamentName || r.tournament || r.tournament_name || r.tourneyName || t.name ||
    r.tournamentId || r.tId || r.tournament_id || t.id
  );
}

function pickTournament(r) {
  const t = r.tournament || r.tourney || r.event || {};
  return {
    name: r.tournamentName || r.tournament || r.tournament_name || r.tourneyName || t.name || "",
    id:   r.tournamentId   || r.tId       || r.tournament_id   || t.id         || "",
  };
}
...
function normalizeFromHome(homeJson) {
  ...
  const recentLogsSrc = safeArray(homeJson.recentLogs || homeJson.logs);
  const recentLogs = recentLogsSrc.map(r => {
    const t = isOnlineTournamentLog(r) ? pickTournament(r) : { name: "", id: "" };
    return {
      dateISO: String(r.date || r.createdAt || r.dateISO || "").slice(0, 10),
      playerDeck: r.deck || r.playerDeck || r.myDeck || r.deckName || "-",
      result: r.result || r.r || "",
              ...
              {(summary?.recentLogs || []).map((log, i) => (
                <tr key={i}>
                  ...
                  <td>
                    {(() => {
                      const tName =
                        log.tournamentName || log.tournament || log.tournament_name ||
                        (log.tournament && log.tournament.name) || (log.event && log.event.name) || "";
                      const tId =
                        log.tournamentId || log.tId || log.tournament_id ||
                        (log.tournament && log.tournament.id) || (log.event && log.event.id) || "";
                      return tName
                        ? (<a href={tId ? `#/tcg-live/torneios/${tId}` : "#/tcg-live/torneios"} className="hover:underline">{tName}</a>)
                        : ("-");
                    })()}
                  </td>
                  <td className={`text-center font-bold ${log.result === 'W' ? 'text-green-400' : log.result === 'L' ? 'text-rose-400' : 'text-zinc-300'}`}>{log.result || "-"}</td>
